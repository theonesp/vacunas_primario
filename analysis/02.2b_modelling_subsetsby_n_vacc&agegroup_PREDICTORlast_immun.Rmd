...
subsetting <- function(data, choose_agegroup, choose_vacc) {
  output <- data %>% 
    ...
    mutate(across(
      c(n_vacunas_covid, ultima_inmun_infecc_bin), droplevels
    ))
  return(output)
}
...
vaccines <- c("Ninguna", "Una", "Dos", "Tres")
...
for (i in seq_along(sub_hosp_list)) {
  output <- sub_hosp_list[[i]] %>%
    filter(str_detect(predictor, "ultima_inmun_infecc"))
...
# write_excel_csv(
#   df_OR_hospitalisation_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups by nvaccines (logreg lastimmun only)/OR_hospitalisation_subgroups_nvacc_PREDICTORtimesincelastimmun2.xls"
# )
```
...
# write_excel_csv(
#   df_OR_intensivecare_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups by nvaccines (logreg lastimmun only)/OR_intensivecare_subgroups_nvacc_PREDICTORtimesincelastimmun2.xls"
# )
```

### Sub-analysis for ICU stay (only hospitalised people)

```{r}
subset_list_only_hosp <- list()
...
for (i in 1:nrow(grid)) {
  ...
  result_dataframe_name <- grid$future_name[i]
  assign(result_dataframe_name, output)
  subset_list_only_hosp[[result_dataframe_name]] <- output
}
...
# Creating a list for storing GAM results
sub_icu_list_only_hosp <- list()

# FOR LOOP: modelling GAMS for each sub-dataframe
for (i in seq_along(subset_list_only_hosp)) {
  # Adjusting the GAM model
  model <- gam(formula_gam_icu, data = subset_list_only_hosp[[i]], family = binomial())
  # Extracting coefficients, confidence intervals and p-values
  coef_data <- data.frame(coef = coef(model))
  se <- sqrt(diag(vcov(model)))
  z_value <- qnorm(0.975)  # 95% confidence level (two-sided)
  ...
  coef_data$p_value <- p_values
  coef_data$predictor <- rownames(coef_data)
  coef_data$original_model <- names(subset_list_only_hosp)[[i]]
  # Storing each results dataframe to our list
  result_dataframe_name <- names(subset_list_only_hosp)[[i]]
  assign(result_dataframe_name, coef_data)
  sub_icu_list_only_hosp[[result_dataframe_name]] <- coef_data
}



df_OR_intensivecare_only_hosp_subgroups <- data.frame()

for (i in seq_along(sub_icu_list_only_hosp)) {
  output <- sub_icu_list_only_hosp[[i]] %>%
    filter(str_detect(predictor, "n_vacunas") | str_detect(predictor, "ultima_inmun"))
  df_OR_intensivecare_only_hosp_subgroups <- bind_rows(df_OR_intensivecare_only_hosp_subgroups, output)
}

df_OR_intensivecare_only_hosp_subgroups %<>% 
  rownames_to_column(var = "nombrefila") %>% 
  select(predictor, original_model, everything(), -nombrefila)

# write_excel_csv(
#   df_OR_intensivecare_only_hosp_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/OR_intensivecare_only_hosp_subgroups_nvacc_lastinmun.xls"
# )
```
...
# write_excel_csv(
#   df_OR_mortality_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups by nvaccines (logreg lastimmun only)/OR_mortality_subgroups_nvacc_PREDICTORtimesincelastimmun2.xls"
# )
```
