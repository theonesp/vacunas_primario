...
# First, dates need to be converted from the original source, missing means NO.
# Also, we convert character variables to get rid of capital letters and accents
pupa <- pupa %>% 
  mutate(
...
pupa <- pupa %>% 
  filter(COD_FEC_INI_PATOLOGIA > ymd(20200229))
# TODO ALVARO SE HA QUEDADO POR AQUI

#TODO FILTER by admission or infection date?

# Convert the dataframe to a data.table since dplyr is not efficient enough
pupa_dt <- setDT(pupa)

# Group by and mutate in a single step
pupa_opt <- pupa_dt[, value := pupa_dt$COD_FEC_INI_PATOLOGIA, by = c("NUHSA_ENCRIPTADO", "DESC_PATOLOGIA_CRONICA")]

# Spread in a single step.
# This will create a new column for each unique element in the DESC_PATOLOGIA_CRONICA column, 
# and fill the cells with a value of 1 for rows where the DESC_PATOLOGIA_CRONICA value matches 
# the column name, and 0 for all other rows.
pupa_opt2 <- dcast(pupa_opt, NUHSA_ENCRIPTADO ~ DESC_PATOLOGIA_CRONICA, value.var = "value", fill = 0)

pupa_opt3 <- 
  dcast(
    pupa_opt,
    NUHSA_ENCRIPTADO ~ DESC_PATOLOGIA_CRONICA,
    value.var = "COD_FEC_INI_PATOLOGIA",
    fill = as.Date(NA)
  )

# Replace the values in the original dataframe with 1 if any value in the row is greater than 0, and 0 otherwise
