...
# estudio_vacunas_t_final_selected_pre_lapply <- readRDS("/opt/datos_compartidos/vacunas_data/Rfiles/toberemoved/estudio_vacunas_t_final_selected_pre_lapply.rds")


# TODO el saveRDS ya esta hecho, queda pte. cargar el pre_lapply y ver si debemos cambiar este chunk porque nos este jorobando variables ya pasadas a factores y demas. Por ahora sigo adelante, metiendo desc_sexo y pautas_covid en las variables a excluir de la funcion lapply
...
fill_cols_to_exclude <- c("desc_sexo","cod_fec_alta","cod_fec_ingreso","covid_first_name","covid_first_date","covid_full_name","covid_full_date","pautas_covid")
fill_cols_to_include <- setdiff(names(estudio_vacunas_t_final_selected), fill_cols_to_exclude)

estudio_vacunas_t_final_selected[, fill_cols_to_include] <- lapply(estudio_vacunas_t_final_selected[, fill_cols_to_include], function(x) ifelse(is.na(x), 0, x))
```
