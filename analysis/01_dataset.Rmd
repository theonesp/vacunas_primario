...
# Fix 5 and 6 shots in that moment was not possible. transcription error.
vacunas_t_covid_pautas <- vacunas_t_covid_pautas %>% 
  mutate(pautas_covid = as.numeric(pautas_covid),
    pautas_covid = case_when(
    pautas_covid >= 4 ~ 4,
    TRUE ~ pautas_covid
  ))

# Comento estas líneas de código ya que no las usamos para el estudio actualmente.
...
```{r}
remove(infect_COVID_first)
remove(infect_COVID_num)
...
#   )
# 
# estudio_vacunas_t_final_selected %>% colnames()

estudio_vacunas_t_final_selected <- estudio_vacunas_t_final_selected %>% 
  mutate(cancer_organo_solido = case_when(
    `Cáncer colorrectal` == 1 ~ 1,
    `Cáncer de bronquio y pulmón`== 1 ~ 1,
    `Cáncer de cabeza y cuello`== 1 ~ 1,
    `Cáncer de cuello uterino`== 1 ~ 1,
    `Cáncer de estómago`== 1 ~ 1,
    `Cáncer de hueso y tejidos blandos`== 1 ~ 1,
    `Cáncer de hígado y vías biliares`== 1 ~ 1,
    `Cáncer de mama`== 1 ~ 1,
    `Cáncer de ovario`== 1 ~ 1,
    `Cáncer de próstata`== 1 ~ 1,
    `Cáncer de páncreas`== 1 ~ 1,
    `Cáncer de riñón y pelvis renal`== 1 ~ 1,
    `Cáncer de testículo`== 1 ~ 1,
    `Cáncer de tiroides`== 1 ~ 1,
    `Cáncer de vejiga`== 1 ~ 1,
    `Cáncer de útero` == 1 ~ 1,
    `Melanoma de piel`== 1 ~ 1
  )) %>% 
  mutate(cancer_organo_solido = replace_na(cancer_organo_solido, 0))


estudio_vacunas_t_final_selected <- estudio_vacunas_t_final_selected %>% 
  mutate(cancer_hematologico = case_when(
    `Leucemia` == 1 ~ 1,
    `Cáncer inmunoproliferativo`== 1 ~ 1,
    `Enfermedad de Hodgkin`== 1 ~ 1,
    `Linfoma no Hodgkin`== 1 ~ 1
  )) %>% 
  mutate(cancer_hematologico = replace_na(cancer_hematologico, 0))
```
...
```{r}
#LIMPIANDO ERRORES FINALES RAFA Y ÁLVARO
# estudio_vacunas_t_final_selected$DESC_SEXO %>% summary()
estudio_vacunas_t_final_selected %>% str()
estudio_vacunas_t_final_selected <- estudio_vacunas_t_final_selected %>% 
  filter(DESC_SEXO == "Mujer" |
           DESC_SEXO == "Hombre")
estudio_vacunas_t_final_selected$DESC_SEXO <- droplevels(estudio_vacunas_t_final_selected$DESC_SEXO)
estudio_vacunas_t_final_selected <- estudio_vacunas_t_final_selected %>% 
...
saveRDS(estudio_vacunas_t_final_selected, 
        file = "/opt/datos_compartidos/vacunas_data/Rfiles/estudio_vacunas_t_final_selected_v13feb.rds")

# feather format is compatible both with R and python
# estudio_vacunas_t_final_selected <- readRDS("/opt/datos_compartidos/vacunas_data/Rfiles/estudio_vacunas_t_final_selected_v13feb.rds")

library(arrow)
write_feather(estudio_vacunas_t_final_selected, "estudio_vacunas_t_final_selected.feather")
write_feather(estudio_vacunas_t_final_selected,
              "/opt/datos_compartidos/vacunas_data/Rfiles/estudio_vacunas_t_final_selected_v13feb.feather")
```
