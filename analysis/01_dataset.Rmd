...
estudio_vacunas_t_final_selected %<>%
  mutate(
    ...
    n_prev_infecc = as.factor(
      case_when(n_prev_infecc == 2 ~ "Dos", n_prev_infecc == 1 ~ "Uno", n_prev_infecc == 0 ~ "Ninguna")
    ),
    ...
    mortality30d = as.factor(
      if_else(fec_fallecimiento - infect_covid_first_date > 30 | is.na(fec_fallecimiento), "No", "Sí")
    ),
...
# saveRDS(variants_week_spain_labels, "/opt/datos_compartidos/vacunas_data/Rfiles/variants_ecdc.rds")
...
estudio_vacunas_t_final_selected$variant_period <- 
  as.factor(estudio_vacunas_t_final_selected$variant_period)

estudio_vacunas_t_final_selected$variant_period <- 
  relevel(estudio_vacunas_t_final_selected$variant_period, ref = "Before α")
