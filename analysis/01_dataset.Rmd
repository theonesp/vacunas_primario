...
estudio_vacunas_t_final_selected %<>%
  mutate(  
    ...
    pautas_covid = factor(pautas_covid, ordered = TRUE)
  ) %>% 
  suppressWarnings()     
...
### Load RDS before *lapply* function (intermediate step, made in order to speed up the process of loading whole 01dataset)

```{r}
# saveRDS(estudio_vacunas_t_final_selected, file = "/opt/datos_compartidos/vacunas_data/Rfiles/toberemoved/estudio_vacunas_t_final_selected_pre_lapply.rds")
estudio_vacunas_t_final_selected <-
  readRDS("/opt/datos_compartidos/vacunas_data/Rfiles/toberemoved/estudio_vacunas_t_final_selected_pre_lapply.rds")


# ALVARO: el saveRDS lo hice para facilitar el analisis de lo que pasaba con la funcion lapply que viene mas adelante -->
# y variables como desc_sexo, pautas_covid o icu_stay_bin (ver slack, dias 24/02/2023 a 26/02/2023). -->
# La funcion lapply estaba coercionando todas las variables a numerico, y eso distorsionaba los datos -->
# e impedia correr las regresiones. En el caso de desc_sexo, esa transformacion impedia un filtrado posterior -->
# que se hacia expresando un termino concreto c("Hombre", "Mujer"). En el caso de icu_stay_bin, el paso de factor a numeric -->
# liaba pardisimo el asunto (para los interesados en saber cómo R interpreta los factores: https://stackoverflow.com/questions/59898810/strange-behaviour-of-as-numeric-with-factor-variable-gives-completely-differ. -->
# El 0 era el primer nivel del factor, por lo que en el paso a numeric pasaba a 1 (de "primer nivel"). --> 
# Los 1 eran el segundo nivel, por lo que pasaban a "2", y los NA a 0 (porque así estaba especificado en el lapply). -->
# Finalmente, dejo el lapply en el chunk siguiente pero modifico la lista de variables fill_cols_to_exclude. Además, -->
# icu_stay_bin la paso a numeric(character) para evitar este fenomeno cuando era coercionada.
# Al final, el problema no es de lapply en si; es el paso factor-->numeric, por cómo interpreta R los factores.
  

estudio_vacunas_t_final_selected <- 
  estudio_vacunas_t_final_selected %>% 
  mutate(icu_stay_bin = as.numeric(as.character(icu_stay_bin)))
```
...
fill_cols_to_exclude <- 
  ...
  select(
    ...
    fec_fallecimiento, age, mortality, asma:vih, cod_fec_ingreso, cod_fec_alta,
...
TODO integrate them in the proper section accordingly
