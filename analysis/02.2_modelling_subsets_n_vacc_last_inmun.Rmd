...
subsetting <- function(data, choose_vacc, choose_time_since_vac) {
  output <- data %>% 
    filter(
...
grid <- 
  expand_grid(vaccines, last_vacc) %>% 
  mutate(
    across(c(vaccines, last_vacc), ~word(.x, 1), .names = "primera_palabra_{.col}"),
    future_name = paste(
...
for (i in seq_along(sub_hosp_list)) {
  output <- sub_hosp_list[[i]] %>%
    filter(str_detect(predictor, "n_vacunas"))
...
# write_excel_csv(
#   df_OR_hospitalisation_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/OR_hospitalisation_subgroups_nvacc_lastinmun.xls"
# )
```
...
# write_excel_csv(
#   df_OR_intensivecare_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/OR_intensivecare_subgroups_nvacc_lastinmun.xls"
# )
```
...
# write_excel_csv(
#   df_OR_mortality_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/OR_mortality_subgroups_nvacc_lastinmun.xls"
# )
```
