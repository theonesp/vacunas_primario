...
gam_hosp_stay_bin <- 
  gam(
    hosp_stay_bin ~ s(age, k = 8) + n_vacunas_covid + desc_sexo + variant_period + 
      ...
      cancer_organo_solido + n_prev_infecc + ultima_inmun_infecc_bin,
    data = train_data,
    family = binomial()
  )
...
variable_order <- 
  rev(c(
    ...
    "Age (group 4: 69-87)", "Age (group 5: 88-120)", "Period before α predominance", "Period of α predominance",
    ...
    "HIV+", "Haematologic cancer", "Solid organ cancer"
  ))
...
filter(predictor != "(Intercept)") %>% 
  mutate(
    predictor = case_when(
      ...
      predictor == "s(age).1" ~ "Age (group 1: 12-30)",
      predictor == "s(age).2" ~ "Age (group 2: 31-49)",
...
asd <- coef_data %>% filter(predictor != "Age (group 4: 69-87)")
# Set up the forest plot using ggplot2
forest_plot <- 
  ggplot(
    coef_data, 
    # %>% filter(predictor != "Age (group 4: 69-87)"), 
    aes(
      x = OR, xmin = OR_lower, xmax = OR_upper,
      y = predictor
      # y = reorder(predictor, -as.numeric(factor(predictor, levels = variable_order)))
    )
  ) +
  ...
  labs(
    x = "Odds Ratio", 
    y = "Predictor", 
    title = "Odds Ratios and Confidence Intervals for Hospital Admission Prediction"
  ) +
  theme_minimal() +
  scale_color_manual(
...
ggsave(
  "/opt/datos_compartidos/vacunas_data/figuras/forest_hospstay_infections_gam_filtered.jpeg",
  device = "jpeg",
  units = "px",
  dpi = 300,
  width = 4000,
...
gam_icu_stay_bin <- 
  gam(
    icu_stay_bin ~ s(age, k = 6) + n_vacunas_covid + desc_sexo + variant_period + 
      asma + cardiopatia_isquemica + cirrosis_hepatica + diabetes + epoc +  hepatopatia_cronica_excepto_cirrosis + 
      hipertension + insuficiencia_cardiaca + insuficiencia_renal_cronica + vih + cancer_hematologico + 
      cancer_organo_solido + n_prev_infecc + ultima_inmun_infecc_bin,
    data = train_data,
...
# Calculate precision, recall, and F1 score
precision <- sum(test_data$predicted_bin == 1 & test_data$icu_stay_bin == 1) / sum(test_data$predicted_bin == 1)
recall <- sum(test_data$predicted_bin == 1 & test_data$icu_stay_bin == 1) / sum(test_data$icu_stay_bin == 1)
f1 <- 2 * precision * recall / (precision + recall)
...
forest_plot <- 
  ggplot(
    coef_data %>% 
      filter(predictor != "Age (group 4: 69-87)" & predictor != "Number of Previous Infections (2)"),
    aes(
      x = OR, xmin = OR_lower, xmax = OR_upper,
      y = predictor
      # y = reorder(predictor, -as.numeric(factor(predictor, levels = variable_order)))
  ...
  labs(
    x = "Odds Ratio", 
    y = "Predictor", 
    title = "Odds Ratios and Confidence Intervals for ICU Admission Prediction"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c("#c0392b", "#27ae60"),
...
# Save the last forest plot rendered
ggsave(
  "/opt/datos_compartidos/vacunas_data/figuras/forest_icustay_infections_gam_filtered.jpeg",
  device = "jpeg",
  units = "px",
  dpi = 300,
  width = 4000,
...
gam_mortality30d <- 
  gam(
    mortality30d ~ s(age, k = 6) + n_vacunas_covid + desc_sexo + variant_period + 
      asma + cardiopatia_isquemica + cirrosis_hepatica + diabetes + epoc +  hepatopatia_cronica_excepto_cirrosis + 
      hipertension + insuficiencia_cardiaca + insuficiencia_renal_cronica + vih + cancer_hematologico + 
      cancer_organo_solido + n_prev_infecc + ultima_inmun_infecc_bin,
    data = train_data,
...
# Calculate precision, recall, and F1 score
precision <- sum(test_data$predicted_bin == 1 & test_data$mortality30d == 1) / sum(test_data$predicted_bin == 1)
recall <- sum(test_data$predicted_bin == 1 & test_data$mortality30d == 1) / sum(test_data$mortality30d == 1)
f1 <- 2 * precision * recall / (precision + recall)
...
forest_plot <- 
  ggplot(
    coef_data %>% 
      filter(
        predictor != "Age (group 4: 69-87)"
        & predictor != "Age (group 2: 31-49)"
        & predictor != "Age (group 5: 88-120)"
  ...
  labs(
    x = "Odds Ratio", 
    y = "Predictor", 
    title = "Odds Ratios and Confidence Intervals for 30-day Mortality Prediction"
  ) +
  theme_minimal() +
  scale_color_manual(
    values = c("#c0392b", "#27ae60"),
...
# Save the last forest plot rendered
ggsave(
  "/opt/datos_compartidos/vacunas_data/figuras/forest_mortality_infections_gam_filtered.jpeg",
  device = "jpeg",
  units = "px",
  dpi = 300,
  width = 4000,
