...
gam_hosp_stay_bin <- gam(
  hosp_stay_bin ~ 
    # s(age, k = 3, bs = "cr")
    s(n_vacunas_covid,  k = 3 ) + age + desc_sexo + asma + cardiopatia_isquemica + cirrosis_hepatica + diabetes + epoc + 
    hepatopatia_cronica_excepto_cirrosis + hipertension + insuficiencia_cardiaca + 
    insuficiencia_renal_cronica + vih + cancer_hematologico + cancer_organo_solido + 
...
# Remove the row with name 's(age).8' from coef_data
coef_data <- subset(coef_data, predictor != 's(age).8')

# Set the predictor variable as a factor with levels in alphabetical order
coef_data$predictor <- factor(coef_data$predictor, levels = sort(coef_data$predictor))

# Define the label names for the predictors
label_names <- c(
  "(Intercept)" = "Intercept",
  "desc_sexoHombre" = "Gender: Male",
  "desc_sexoMujer" = "Gender: Female",
  "asmaSí" = "Asthma: Yes",
  "cardiopatia_isquemicaSí" = "Ischemic Heart Disease: Yes",
  "cirrosis_hepaticaSí" = "Hepatic Cirrhosis: Yes",
  "diabetesSí" = "Diabetes: Yes",
  "epocSí" = "COPD: Yes",
  "hepatopatia_cronica_excepto_cirrosisSí" = "Chronic Hepatic Disease (except Cirrhosis): Yes",
  "hipertensionSí" = "Hypertension: Yes",
  "insuficiencia_cardiacaSí" = "Heart Failure: Yes",
  "insuficiencia_renal_cronicaSí" = "Chronic Renal Insufficiency: Yes",
  "vihSí" = "HIV: Yes",
  "cancer_hematologicoSí" = "Hematologic Cancer: Yes",
  "cancer_organo_solidoSí" = "Solid Organ Cancer: Yes",
  "n_vacunas_covidDos" = "Number of COVID Vaccines: 2",
  "n_vacunas_covidTres" = "Number of COVID Vaccines: 3",
  "n_vacunas_covidUna" = "Number of COVID Vaccines: 1",
  "n_prev_infeccDos" = "Number of Previous Infections: 2",
  "n_prev_infeccUno" = "Number of Previous Infections: 1",
  "ultima_inmun_infecc_binOne to three months" = "Last Immunization/Infection: One to Three Months Ago",
  "ultima_inmun_infecc_binSix months or more" = "Last Immunization/Infection: Six Months or More Ago",
  "ultima_inmun_infecc_binThree to six months" = "Last Immunization/Infection: Three to Six Months Ago",
  "s(age).1" = "Age Bin Nº 1",
  "s(age).2" = "Age Bin Nº 2",
  "s(age).3" = "Age Bin Nº 3",
  "s(age).4" = "Age Bin Nº 4",
  "s(age).5" = "Age Bin Nº 5",
  "s(age).6" = "Age Bin Nº 6",
  "s(age).7" = "Age Bin Nº 7",
  "s(age).9" = "Age Bin Nº 9"
)

# Assign the labels to the predictor levels
levels(coef_data$predictor) <- label_names[levels(coef_data$predictor)]

# Set up the forest plot using ggplot2
forest_plot <- ggplot(coef_data, aes(x = OR, xmin = OR_lower, xmax = OR_upper, y = predictor)) +
  geom_point(size = 2, color = "#2c3e50") +
  geom_errorbarh(height = 0.2, color = "#2c3e50")  +
  geom_vline(xintercept = 1, linetype = "dashed") +
  labs(x = "Odds Ratio", y = "Predictor", title = expression("Forest Plot of Odds Ratios and Confidence Intervals for" ~ 
                                                               "Hospital Admission Prediction")) +
  theme_minimal() +
  scale_color_manual(
    values = c("#27ae60", "#c0392b"),
    breaks = c("#27ae60", "#c0392b"),
...
forest_plot <- ggplot(coef_data, aes(x = OR, xmin = OR_lower, xmax = OR_upper, y = predictor)) +
  geom_point(size = 2, color = "#2c3e50") +
  geom_errorbarh(height = 0.2, color = "#2c3e50")  +  # Increased whisker width
  geom_vline(xintercept = 1, linetype = "dashed") +
  labs(x = "Odds Ratio", y = "Predictor", title = expression("Odds Ratios and Confidence Intervals for Hospital Admission Prediction")) +
  theme_minimal() +
  scale_color_manual(
    values = c("#c0392b", "#27ae60"),  # Exchanged colors
    breaks = c("#c0392b", "#27ae60"),
    labels = c("Adverse", "Protective"),
    name = "Effect on Outcome"  # Changed legend title
  )
...
p <- plot(gam_hosp_stay_bin)

# Check the length of the vectors
length_x <- length(p[[1]]$x)
length_y <- length(p[[1]]$raw)
length_se <- length(p[[1]]$se)

# Ensure all vectors have the same length
min_length <- min(length_x, length_y, length_se)
p[[1]]$x <- p[[1]]$x[1:min_length]
p[[1]]$raw <- p[[1]]$raw[1:min_length]
p[[1]]$se <- p[[1]]$se[1:min_length]

# Convert plot data to a dataframe
plot_data <- data.frame(x = p[[1]]$x, y = p[[1]]$raw, se = p[[1]]$se)

# Calculate upper and lower bounds of the confidence interval
plot_data$upper <- plot_data$y + 1.96 * plot_data$se
plot_data$lower <- plot_data$y - 1.96 * plot_data$se

# Plot the data
ggplot(plot_data, aes(x = x, y = y)) +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.3) +
  geom_line() +
  labs(x = "X Axis", y = "Chances of Suffering the Outcome") +
  theme_minimal()
