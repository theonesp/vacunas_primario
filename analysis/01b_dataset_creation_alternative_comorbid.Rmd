...
# ALVARO: to consider whether each comorbidity was diagnosed prior to the infection, one must start by not executing previous code line (otherwise, date info will be lost). I propose a way of doing so, that you may find in line 358.
```
...
episodios_cmbd <- fread("/opt/datos_compartidos/vacunas_data/02_Episodios_CMBD.txt",header = T,encoding = 'UTF-8');episodios_cmbd<-as.data.frame(episodios_cmbd)
...
```{r}
vacunas <- fread("/opt/datos_compartidos/vacunas_data/03_Vacunas.txt",header = T,encoding = 'UTF-8');vacunas<-as.data.frame(vacunas)
# dates need to be converted from the original source, missing means NO
...
# ALVARO: code line #105 is commented out. Instead, I propose bringing here poblacion_diana_opt (without executing #105), joining it with the rest of the datasets (as seen in code just above) and using infect_COVID_first_date to guess which comorbidity was present upon infection. We could design something similar for cancer, in order to discard those diagnosed after COVID infection. Also, this process refines NAs.
comorbidities <- c("VIH","EPOC","ASMA","INSUF_CARDIACA","CARDIOPATÍA_ISQUÉMCIA","HEPATOPATIA_CRÓNICA","CIRROSIS","INSUF_RENAL_CRÓNICA")
estudio_vacunas_final <- 
  estudio_vacunas_final %>% 
