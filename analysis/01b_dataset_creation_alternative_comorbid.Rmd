...
# ALVARO: to consider whether each comorbidity was diagnosed prior to the infection, one must start by not executing previous code line (otherwise, date info will be lost).
```
...
# ALVARO: code line #105 is commented out. Instead, I propose bringing here poblacion_diana_opt (without executing #105), joining it with the rest of the datasets and using infect_COVID_first_date to guess which comorbidity was present upon infection. We could design something similar for cancer, in order to discard those diagnosed after COVID infection. Also, this process refines NAs.
