...
subsetting <- function(data, choose_agegroup, choose_variant) {
  output <- data %>% 
    filter(
    ...
    mutate(across(
      c(age_fct_3groups, variant_period), droplevels
    ))
  return(output)
}
...
for (i in seq_along(sub_hosp_list)) {
  output <- sub_hosp_list[[i]] %>%
    filter(str_detect(predictor, "ultima_inmun"))
...
# write_excel_csv(
#   df_OR_hospitalisation_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups3 by variant & agegroup (logreg lastimmun only)/OR_hospitalisation_subgroups_timesincelastinmunnewgroups.xls"
# )
...
# write_excel_csv(
#   df_OR_intensivecare_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups3 by variant & agegroup (logreg lastimmun only)/OR_intensivecare_subgroups_timesincelastinmunnewgroups.xls"
# )
...
# write_excel_csv(
#   df_OR_mortality_subgroups,
#   file = "/opt/datos_compartidos/vacunas_data/tablas/groups3 by variant & agegroup (logreg lastimmun only)/OR_mortality_subgroups_timesincelastinmunnewgroups.xls"
# )
