...
# Due to the low amount of rows in this dataset, I perform some operations with tidyverse only

prueba1 <- setDT(infect_covid_plusoneinf_raw) %>% 
  lazy_dt(immutable = FALSE) %>% 
  pivot_longer(
    1:(ncol(.) - 1),
    names_to = "orden_pcr",
  ...
  mutate(new_diff = as.numeric(fecha_pcr - lag(fecha_pcr, n = 1))) %>% 
