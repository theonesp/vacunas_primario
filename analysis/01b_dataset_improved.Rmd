...
vacunas3 <- setDT(vacunas2) %>% 
  ...
  group_by(NUHSA_ENCRIPTADO, orden_infeccion) %>%
  mutate(
    fec_ultima_vac = max(FEC_VACUNACION),
    fec_ultima_inmun = max(fec_ultima_vac, unique(prev_infect_date), na.rm = TRUE)
  ) %>% 
  ungroup() %>% 
  mutate(
  ...
  mutate(
    ...
    ultima_vac_infecc_mas6meses_bin = if_else(ultima_vac_infecc_dias > 182, 1, 0),
    ultima_inmun_infecc_meses_quant = days(ultima_inmun_infecc_dias) / months(1),
    ultima_inmun_infecc_mas6meses_bin = if_else(ultima_inmun_infecc_dias > 182, 1, 0)
    # 182 == mean days of 6 months
  ) %>%
  mutate(
  ...
  summarise(
    ...
    ultima_vac_infecc_mas6meses_bin = unique(ultima_vac_infecc_mas6meses_bin),
    ultima_vac_infecc_meses_quant = unique(ultima_vac_infecc_meses_quant2),
    ultima_inmun_infecc_mas6meses_bin = unique(ultima_inmun_infecc_mas6meses_bin),
...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(
    ...
    ultima_inmun_infecc_dias = if_else(
      n_vacunas_covid == 0, infect_covid_first_date - prev_infect_date, 999
    ),
    ultima_inmun_infecc_meses_quant = if_else(
      n_vacunas_covid == 0, trunc(days(ultima_inmun_infecc_dias) / months(1)), ultima_inmun_infecc_meses_quant
    ),
    ultima_inmun_infecc_mas6meses_bin = if_else(
      n_vacunas_covid == 0, if_else(ultima_inmun_infecc_dias > 182, 1, 0), ultima_inmun_infecc_mas6meses_bin
    ),
    # 182 == mean days of 6 months
  ) %>% 
...
n_cases_under18yo_more2doses <- estudio_vacunas_t_final_selected %>% 
  filter(age < 18) %>% 
  filter(n_vacunas_covid %in% c("Tres", "Cuatro", "Cinco")) %>% 
  nrow(.)
...
variants_ordered <- pull(variants_totalpercents_ranking, who_label_family)
...
variants_week_spain_labels %>% 
  ...
  theme_bw() + 
  theme(
    axis.text.x = element_text(angle = 90, size = 3.5, hjust = 1, vjust = 0.5),
...
begining_alpha <- ymd(20210221) # as it is the monday of isoweek 2021-07, when alpha overcame 50% of sequencing
begining_delta <- ymd(20210704)
begining_omicron <- ymd(20211226)
end_omicron <- ymd(20220331)
...
estudio_vacunas_t_final_selected %<>% 
  mutate(
   variant_period = case_when(
     infect_covid_first_date >= begining_alpha & infect_covid_first_date < begining_delta ~ "Alpha",
     infect_covid_first_date >= begining_delta & infect_covid_first_date < begining_omicron ~ "Delta",
     infect_covid_first_date >= begining_omicron & infect_covid_first_date < end_omicron ~ "Omicron",
     TRUE ~ "Before Alpha predominance (prior to Feb 21st, 2021)"
   ) 
  )
```
