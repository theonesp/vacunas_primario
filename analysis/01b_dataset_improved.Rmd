...
pupa2 <- setDT(pupa_dt) %>% 
  ...
  mutate(dummy_indic = 1) %>%  # TODO vas por aquí ÁLVARO
  pivot_wider(
...
vacunas3 <- setDT(vacunas2) %>% 
  ...
  mutate(
    ...
    inmun_menos6meses_bin = if_else(ultima_vac_infecc_dias < 183, 1, 0) # 182 == mean days of 6 months
  ) %>%
  mutate(inmun_meses_quant2 = trunc(inmun_meses_quant)) %>% 
  group_by(NUHSA_ENCRIPTADO, orden_infeccion) %>%
  summarise(
    pautas_covid = n(),
    inmun_menos6meses_bin = unique(inmun_menos6meses_bin),
...
infect_bdu <- setDT(infect_covid5) %>% 
  lazy_dt() %>% 
  left_join(poblacion_diana3, by = "NUHSA_ENCRIPTADO") %>% 
  as.data.frame()
# unir estas dos partes?
infect_bdu_pupa <- setDT(infect_bdu) %>% 
  lazy_dt() %>% 
...
# TODO recuerda hacer los join uno a uno para unir bien según tabla de datos



# TODO
# CUIDADO AQUI HACER EL JOIN COMORBILIDADES, INGRESOS Y VACUNAS POR NUHSA Y ORDEN INFECCION 
# PARA NO PERDER LA GRANULARIDAD DE LAS COMORBILIDADES AJUSTADAS!!
