...
episodios_cmbd_t <- fread("/opt/datos_compartidos/vacunas_data/total/ENC_VACUNAS_CMBD.txt",header = T,encoding = 'UTF-8');cmbd_t<-as.data.frame(episodios_cmbd_t);
...
# dates need to be converted from the original source, missing means NO
episodios_cmbd_t$COD_FEC_INGRESO <-as.Date(as.character(episodios_cmbd_t$COD_FEC_INGRESO ),format="%Y%m%d")

episodios_cmbd_t$COD_FEC_ALTA<-as.Date(as.character(episodios_cmbd_t$COD_FEC_ALTA),format="%Y%m%d")
