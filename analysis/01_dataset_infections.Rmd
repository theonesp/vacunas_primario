...
estudio_vacunas_t_final_selected %<>%
  ...
  mutate(
    ...
    ultima_inmun_infecc_dias = if_else(
      ...
      infect_covid_first_date - prev_infect_date,
...
#re-coding number of shots

# Overwrite the values in the column with the mapped values
estudio_vacunas_t_final_selected$n_vacunas_covid <- recode(estudio_vacunas_t_final_selected$n_vacunas_covid,
                                                            "Ninguna" = 0,
                                                            "Una" = 1,
                                                            "Dos" = 2,
                                                            "Tres" = 3,
...
estudio_vacunas_t_final_selected$n_vacunas_covid<-as.numeric((estudio_vacunas_t_final_selected$n_vacunas_covid))

estudio_vacunas_t_final_selected$n_vacunas_covid<-as.factor((estudio_vacunas_t_final_selected$n_vacunas_covid))


age_breaks <- c(18, 24, 35, 45, 60, 70, 80, 120)
# Establezco estos puntos de corte pensando en los puntos de inflexión vitales que hacen que cambien tus...
# actividades, tu nivel de exposición y tu percepción de riesgo
month(ymd(20210301))
...
estudio_vacunas_t_final_selected %<>% 
  mutate(
    ...
    # across(comorb_no_cancer, ~relevel(.x, ref = "No")),
  # NEW
    age_breaks = case_when(
   between(age, 0, 18) ~  "edad 0 a 18",
  between(age, 19, 24) ~  "edad 19 a 24",
  between(age, 25, 35) ~  "edad 25 a 35",
  between(age, 36, 45) ~  "edad 36 a 45",
  between(age, 46, 60) ~  "edad 46 a 60",
  between(age, 61, 70) ~  "edad 61 a 70",
  between(age, 71, 80) ~  "edad 71 a 80",
  between(age, 81, 120) ~ "edad 81 a 120",
  TRUE ~ NA_character_
  )
)
